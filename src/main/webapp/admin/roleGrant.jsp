...
<script type="text/javascript">
	var privilegeTree;
	$(function(){
		privilegeTree = $('#privilegeTree').tree({
			url : '${pageContext.request.contextPath}/privilegeController/allTree',
			parentField : 'pid',
			checkbox : true,
			onClick : function(node) {
			},
			onLoadSuccess : function(node, data){
				var ids = $.stringToList('${role.privilegeIds}');
				if (ids.length > 0) {
					for ( var i = 0; i < ids.length; i++) {
						if (privilegeTree.tree('find', ids[i])) {
							privilegeTree.tree('check', privilegeTree.tree('find', ids[i]).target);
						}
					}
				}
				$('#roleGrantLayout').layout('panel', 'west')
						.panel('setTitle', $.formatString('[{0}]角色可以访问的资源', '${role.name}'));
		...
		$('#form').form({
			...
			onSubmit : function() {
				...
				var checknodes = privilegeTree.tree('getChecked');
				var ids = [];
	...
	function checkAll() {
		var nodes = privilegeTree.tree('getChecked', 'unchecked');
		if (nodes && nodes.length > 0) {
			for ( var i = 0; i < nodes.length; i++) {
				privilegeTree.tree('check', nodes[i].target);
			}
		}
	}
	...
	function uncheckAll() {
		...
		if (nodes && nodes.length > 0) {
			for ( var i = 0; i < nodes.length; i++) {
				privilegeTree.tree('uncheck', nodes[i].target);
			}
		}
	}
	
	function checkInverse() {
		var unchecknodes = privilegeTree.tree('getChecked', 'unchecked');
		var checknodes = privilegeTree.tree('getChecked');
		if (unchecknodes && unchecknodes.length > 0) {
			for ( var i = 0; i < unchecknodes.length; i++) {
				privilegeTree.tree('check', unchecknodes[i].target);
			}
		}
		if (checknodes && checknodes.length > 0) {
			for ( var i = 0; i < checknodes.length; i++) {
				privilegeTree.tree('uncheck', checknodes[i].target);
			}
		}
	}
	...
	<div data-options="region:'west'" title="系统资源" style="width: 300px; padding: 1px;">
		<div class="well well-small">
			<form id="form" method="post">
				...
				<input id="resourceIds" name="privilegeIds" type="hidden" />
			</form>
		</div>
	</div>
