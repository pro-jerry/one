...
<html>
  <head>
    ...
    <c:if test="${fn:contains(sessionInfo.privilegeList,'/roleController/editPage')}">
    	<script type="text/javascript">
    		$.canEdit = true;
    	</script>
    </c:if>
    <c:if test="${fn:contains(sessionInfo.privilegeList,'/roleController/delete')}">
    	<script type="text/javascript">
    		$.canDelete = true;
    	</script>
    </c:if>
    <c:if test="${fn:contains(sessionInfo.privilegeList,'/roleController/grantPage')}">
    	<script type="text/javascript">
    		$.canGrant=true;
    	</script>	
    </c:if>
    		...
    		treeGrid=$('#treeGrid').treegrid({
    			...
    			columns:[[{
    				...
    				formatter:function(value,row){
    					if(value){
    						return row.privilegeNames;
    					}
    					return '';
    				}
    			},{
    				field : 'privilegeNames',
    				title : '拥有资源名称',
    				width : 80,
    				hidden : true
    			},{
    				...
    				formatter:function(value,row,index){
    					var  str='';
    					if($.canEdit){
    						str += $.formatString('<img onclick="editFun(\'{0}\');" src="{1}" title="编辑"/>',
    						row.id,'${pageContext.request.contextPath}/style/images/extjs_icons/pencil.png');
    					}
    					str+='&nbsp;';
    					if($.canGrant){
    						str+=$.formatString('<img onclick="grantFun(\'{0}\');" src="{1}" title="授权"/>',
    						row.id,'${pageContext.request.contextPath}/style/images/extjs_icons/key.png');
    					}
    					str += '&nbsp;';
    					if($.canDelete){
  ...
  <body>
    ...
    <div id="toolbar" style="display: none;">
    	<c:if test="${fn:contains(sessionInfo.privilegeList, '/roleController/addPage')}">
    		...
    		<a onclick="undo()" href="javascript:void(0);" class="easyui-linkbutton"
    		data-options="plain:true,iconCls:'resultset_previous'">折叠</a>
    		<a onclick="treeGrid.treegrid('load');" href="javascript:void(0);" class="easyui-linkbutton"
